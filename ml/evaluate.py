...
def weighted_f1(y_true, y_pred, weights):
    ...
    y_true_converted = [ast.literal_eval(label) if isinstance(
        label, str) else label for label in y_true]
    y_pred_converted = [ast.literal_eval(label) if isinstance(
        label, str) else label for label in y_pred]
...
ref_uri = 'data/gt_test.csv'
pred_uri = 'data/submission.csv'
class_weights = {'B-discount': 1, 'B-value': 2, 'I-value': 2, 'O': 0.003}

metrics = evaluate(ref_uri, pred_uri, class_weights)
